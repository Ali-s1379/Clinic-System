...
{
    public partial class AllMedicalHistoryForm : Form
    {
        ...
        public AllMedicalHistoryForm()
        {
            InitializeComponent();
        }

        private void AllMedicalHistoryForm_Load(object sender, EventArgs e)
        {
            try
            {
                SqlConnection cnn;
                ...
                SqlDataAdapter adp = new SqlDataAdapter("SELECT * FROM medical_history", cnn);
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string date = dr[1].ToString().Substring(0, dr[1].ToString().IndexOf(' '));
        ...
        private void btnFilter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                listView1.Items.Clear();
                string sql = "";
                if (txtPatientId.Text == "" && txtDate.Text != "")
                {
                    string date = txtDate.Text;
                    date = Jalali_to_gregorian(date);
                    sql = "select * from medical_history where date_medical_history > '" + date + "'";
                }
                else if (txtDate.Text == "" && txtPatientId.Text != "")
                {
                    sql = "select * from medical_history where patient_id = " + txtPatientId.Text;
                }
                else if (txtPatientId.Text != "" && txtDate.Text != "")
                {
                    string date = txtDate.Text;
                    date = Jalali_to_gregorian(date);
                    sql = "select * from medical_history where date_medical_history > '" + date + "' AND patient_id = " + txtPatientId.Text;
                }
                else sql = "select * from medical_history";
